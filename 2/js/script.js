...
window.addEventListener('load', () => {
  ...
  const fadeOut = setInterval(() => {
    const currentOpacity = parseFloat(preloader.style.opacity);
    if(currentOpacity > 0){
      preloader.style.opacity = currentOpacity - 0.05;
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tabs.forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      setTimeout(() => {
        ...
        // Initialize counters inside the new tab
  ...
  // Initialize counters in default active tab
